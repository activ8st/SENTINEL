...
export default function MarketingNavbar({ onOpenWaitlist }) {
  ...
  const getLinkClass = (path) => {
    return location.pathname === path 
      ...
      : "text-slate-800 dark:text-gray-200 hover:text-[#10b981] dark:hover:text-[#10b981] transition-colors";
  };
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full border-b border-black/10 dark:border-white/10 bg-white/95 dark:bg-[#050505]/95 backdrop-blur-2xl text-gray-900 dark:text-white transition-colors duration-300">
      <nav className="max-w-7xl mx-auto px-5 sm:px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-black/10 dark:border-white/10 hover:border-[#10b981] text-xs font-semibold text-gray-800 dark:text-gray-200 transition-colors"
        ...
        <div className="flex md:hidden items-center gap-3">
          ...
          <button
            ...
            className="p-2.5 rounded-xl border border-slate-300 dark:border-white/15 bg-slate-100 dark:bg-white/10 text-slate-900 dark:text-white transition-colors"
            aria-label="Toggle Menu"
          >
            {mobileMenuOpen ? <X className="w-6 h-6 text-[#10b981]" /> : <Menu className="w-6 h-6 text-slate-900 dark:text-white" />}
          </button>
        </div>
      </nav>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 dark:border-white/10 bg-slate-50/98 dark:bg-[#080c0a]/98 backdrop-blur-3xl px-6 py-6 space-y-5 animate-in slide-in-from-top duration-300 shadow-2xl">
          <div className="flex flex-col space-y-4 text-base font-bold">
            <Link 
              to="/LandingPage" 
              className={`flex items-center justify-between py-2.5 border-b border-slate-200 dark:border-white/5 ${getLinkClass('/LandingPage')}`}
            ...
            <Link 
              to="/Platform" 
              className={`flex items-center justify-between py-2.5 border-b border-slate-200 dark:border-white/5 ${getLinkClass('/Platform')}`}
              onClick={() => setMobileMenuOpen(false)}
            >
            ...
            <Link 
              to="/Manifesto" 
              className={`flex items-center justify-between py-2.5 border-b border-slate-200 dark:border-white/5 ${getLinkClass('/Manifesto')}`}
              onClick={() => setMobileMenuOpen(false)}
            >
            ...
            <Link 
              to="/Contact" 
              className={`flex items-center justify-between py-2.5 border-b border-slate-200 dark:border-white/5 ${getLinkClass('/Contact')}`}
              onClick={() => setMobileMenuOpen(false)}
            >
          ...
          <div className="pt-3 flex items-center justify-between border-t border-slate-200 dark:border-white/10">
            <button
              onClick={() => changeLang(lang === 'it' ? 'en' : 'it')}
              className="flex items-center gap-2 px-4 py-2 rounded-full border border-slate-300 dark:border-white/15 bg-white dark:bg-white/5 text-slate-800 dark:text-white text-xs font-bold transition-colors"
            >
